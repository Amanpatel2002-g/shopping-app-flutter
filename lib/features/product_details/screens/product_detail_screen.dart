...
import 'package:amazon_new/constants/stars.dart';
import 'package:amazon_new/features/product_details/services/product_details_services.dart';
import 'package:amazon_new/features/screens/search/screens/search_screen.dart';
...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.product.ratings!.length; i++) {
      totalRatings += widget.product.ratings![i].ratings.toDouble();
      if (widget.product.ratings![i].userId ==
          Provider.of<UserProvider>(context, listen: false).user.id) {
        myratings = widget.product.ratings![i].ratings.toDouble();
      }
    }
    if (totalRatings != 0) {
      avgRating = totalRatings / widget.product.ratings!.length;
    }
  }
            ...
            RatingBar.builder(
                ...
                onRatingUpdate: (rat) {
                  productDetailsServices.rateProduct(
                      context: context, product: widget.product, ratings: rat);
                }),
          ],
        ),
      ),
